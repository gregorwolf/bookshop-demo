...
entity GWSAMPLE_BASIC.BusinessPartnerSet {
  ...
  key BusinessPartnerID   : String(10) not null;
      ...
      ToSalesOrders       : Association to many GWSAMPLE_BASIC.SalesOrderSet
                              on ToSalesOrders.CustomerID = BusinessPartnerID;

      @cds.ambiguous  : 'missing on condition?'
      ToContacts          : Association to many GWSAMPLE_BASIC.ContactSet
                              on ToContacts.BusinessPartnerID = BusinessPartnerID;

      @cds.ambiguous  : 'missing on condition?'
      ToProducts          : Association to many GWSAMPLE_BASIC.ProductSet
                              on ToProducts.SupplierID = BusinessPartnerID;
};
...
entity GWSAMPLE_BASIC.ProductSet {
  ...
  key ProductID             : String(10) not null;
      ...
      @cds.ambiguous  : 'missing on condition?'
      ToSupplier            : Association to GWSAMPLE_BASIC.BusinessPartnerSet
                                on ToSupplier.BusinessPartnerID = SupplierID;

      @cds.ambiguous  : 'missing on condition?'
      ToSalesOrderLineItems : Association to many GWSAMPLE_BASIC.SalesOrderLineItemSet
                                on ToSalesOrderLineItems.ProductID = ProductID;
};
...
entity GWSAMPLE_BASIC.SalesOrderSet {
  ...
  key SalesOrderID               : String(10) not null;
      ...
      @cds.ambiguous  : 'missing on condition?'
      ToBusinessPartner          : Association to GWSAMPLE_BASIC.BusinessPartnerSet
                                     on ToBusinessPartner.BusinessPartnerID = CustomerID;

      @cds.ambiguous  : 'missing on condition?'
      ToLineItems                : Association to many GWSAMPLE_BASIC.SalesOrderLineItemSet
...
entity GWSAMPLE_BASIC.SalesOrderLineItemSet {
  ...
  key ItemPosition : String(10) not null;
      ...
      @cds.ambiguous  : 'missing on condition?'
      ToHeader     : Association to GWSAMPLE_BASIC.SalesOrderSet
                       on ToHeader.SalesOrderID = SalesOrderID;

      @cds.ambiguous  : 'missing on condition?'
      ToProduct    : Association to GWSAMPLE_BASIC.ProductSet
...
entity GWSAMPLE_BASIC.ContactSet {
  ...
  key ContactGuid       : UUID not null;
      ...
      @cds.ambiguous  : 'missing on condition?'
      ToBusinessPartner : Association to GWSAMPLE_BASIC.BusinessPartnerSet
                            on ToBusinessPartner.BusinessPartnerID = BusinessPartnerID;
};
...
@cds.external: true
action GWSAMPLE_BASIC.RegenerateAllData(NoOfSalesOrders : Integer) returns GWSAMPLE_BASIC.CT_String;
